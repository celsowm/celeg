...
SOURCE = ROOT / "apps" / "benchmark" / "metal" / "prefill_tail_fast.mm"
...
def compile_harness(build_dir: pathlib.Path) -> pathlib.Path:
    ...
    binary = build_dir / "celeg-metal-prefill-tail-benchmark"
    run([
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Benchmark remaining LFM2.5 pp512 Metal RMSNorm and SwiGLU costs."
    )
