...
def main() -> int:
    ...
    neutral_model_paths = (
        "include/celeg/model/config",
        "include/celeg/model/execution",
    ...
    for relative in neutral_model_paths:
        ...
        for path in candidates:
            ...
            if "celeg/backend/cuda" in text or "celeg/detail" in text:
                errors.append(f"backend/internal include in public-neutral header: {path}")

    for relative in ("include/celeg/checkpoint",):
    ...
    legacy = re.compile(r"\b(?:CelegModel|CelegInferenceSession|CelegDiagnostics|CelegPersistence|IPackedSession)\b")
