...
#define LFM_CUDA(call) ::lfm::check_cuda((call), #call)
#define LFM_CUBLAS(call) ::lfm::check_cublas((call), #call)
#define LFM_KERNEL_CHECK() ::lfm::check_cuda(cudaPeekAtLastError(), "CUDA kernel launch")

#if defined(LFM_DEBUG_SYNC) && LFM_DEBUG_SYNC
#define LFM_KERNEL_DEBUG_SYNC(stream)                    \
    do {                                                 \
        LFM_KERNEL_CHECK();                              \
        ::lfm::check_cuda(cudaStreamSynchronize(stream), \
...
class CudaStream {
public:
    CudaStream() { LFM_CUDA(cudaStreamCreateWithFlags(&stream_, cudaStreamNonBlocking)); }
    ~CudaStream() {
...
public:
    explicit CudaEvent(unsigned flags = cudaEventDefault) {
        LFM_CUDA(cudaEventCreateWithFlags(&event_, flags));
    }
    ~CudaEvent() {
    ...
    void record(cudaStream_t stream) { LFM_CUDA(cudaEventRecord(event_, stream)); }
    void synchronize() { LFM_CUDA(cudaEventSynchronize(event_)); }
    cudaEvent_t get() const { return event_; }

    static float elapsed_ms(const CudaEvent& start, const CudaEvent& stop) {
        float result = 0.0f;
        LFM_CUDA(cudaEventElapsedTime(&result, start.get(), stop.get()));
        return result;
    }
...
public:
    CublasLtHandle() { LFM_CUBLAS(cublasLtCreate(&handle_)); }
    ~CublasLtHandle() {
...
public:
    explicit CublasHandle(cudaStream_t stream) {
        LFM_CUBLAS(cublasCreate(&handle_));
        try {
            LFM_CUBLAS(cublasSetStream(handle_, stream));
            LFM_CUBLAS(cublasSetMathMode(handle_, CUBLAS_TENSOR_OP_MATH));
        } catch (...) {
...
public:
    ...
    void zero_async(cudaStream_t stream) {
        if (ptr_ && count_) LFM_CUDA(cudaMemsetAsync(ptr_, 0, bytes(), stream));
    }
...
public:
    ...
    void capture_begin(cudaStream_t stream) {
        ...
        reset();
        LFM_CUDA(cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal));
        capturing_ = true;
    }
    ...
    void capture_end(cudaStream_t stream) {
        ...
        graph_ = captured;
        try {
            LFM_CUDA(cudaGraphInstantiate(&exec_, graph_, nullptr, nullptr, 0));
        } catch (...) {
            reset();
            throw;
        }
    ...
    void launch(cudaStream_t stream) const {
        ...
        LFM_CUDA(cudaGraphLaunch(exec_, stream));
    }
