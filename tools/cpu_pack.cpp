...
int main(int argc, char** argv) {
    try {
        if (argc < 2 || argc > 5) {
            std::cerr << "usage: celeg-cpu-pack MODEL_SAFETENSORS [CACHE_DIR] [GROUP=32|64] [ISA=auto|scalar|avx2|neon]\n";
            return 2;
        }
        ...
        if (argc >= 4) {
            ...
            options.weight_format = group == 64 ? celeg::CpuWeightFormat::Q4Group64
                                                : celeg::CpuWeightFormat::Q4Group32;
        }
        if (argc >= 5) options.isa = celeg::parse_cpu_isa(argv[4]);
        options.use_pack_cache = true;
        options.threads = 1;
        celeg::GenerationConfig generation;
        celeg::CpuModel model(argv[1], 1, options, generation);
