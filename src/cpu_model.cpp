...
CpuModel::CpuModel(std::unique_ptr<Impl> impl) : impl_(std::move(impl)) {
...
void CpuModel::reset() { impl_->reset(); }

void CpuModel::prefill(const std::vector<int32_t>& tokens) {
...
void CpuModel::set_generation_config(GenerationConfig generation) {
    impl_->set_generation(generation);
}

std::vector<float> CpuModel::copy_logits() const { return impl_->logits; }
RuntimeMetrics CpuModel::runtime_metrics() const { return impl_->metrics; }
...
CpuIsa CpuModel::isa() const { return impl_->shared->options.isa; }
CpuKvCacheMode CpuModel::kv_cache_mode() const {
    return impl_->shared->options.kv_cache_mode;
}

std::string CpuModel::backend_description() const {
    std::ostringstream out;
...
const std::filesystem::path& CpuModel::pack_path() const {
    return impl_->shared->pack_file;
}

bool CpuModel::loaded_from_pack() const { return impl_->shared->loaded_pack; }

CpuPrefixSnapshot CpuModel::export_prefix_snapshot() const {
    return impl_->export_prefix_snapshot();
}

void CpuModel::restore_prefix_snapshot(CpuPrefixSnapshot snapshot,
                                       bool ready_for_decode) {
...
uint64_t CpuModel::attention_parallel_calls() const {
