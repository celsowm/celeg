...
// Bulk (batched) prefill.
//
// This used to be one ~600 line function.  It is split along the same execution
// seams the host token path uses (docs/SOLID_REVIEW_BACKENDS.md §5.1):
//
//     layer iteration                 run_prefill_layers / run_prefill_layer
//     mixer execution                 run_prefill_mixer + run_prefill_<mixer>
//     attention capability selection  prefill_attention_plan
//     KV access policy                store_and_attend_prefill
//     feed-forward execution          run_mlp_prefill (pre-existing)
//     state transition                prefill_batched's epilogue
//
// Bulk prefill has only one KV addressing mode (contiguous, host-scalar
// position), so it takes no KV policy value; the split is about the mega
// function, not about paged/contiguous unification.
...
// ---------------------------------------------------------------------------
// KV access policy: append the prompt's K/V and score it
// ---------------------------------------------------------------------------
...
void CudaCompiledModel::store_and_attend_prefill(
    ...
    // Prefill is a bulk launch: the extent is the query row count, and the
    // causal prefix each row attends to is derived from the row index.
    ...
    // The tiled and batched-GEMM paths read query/KV straight out of the packed
    // activation buffers, so they need the interleaved-head row strides.
...
void CudaCompiledModel::run_prefill_latent_attention(
    ...
    // branch below does NOT return; the non-factorized latent path underneath
    // it runs afterwards in both cases.  See the report accompanying this
    // refactor -- fixing it is a behavior change and out of scope here.
...
// ---------------------------------------------------------------------------
// Mixer execution and layer iteration
// ---------------------------------------------------------------------------
...
void CudaCompiledModel::run_prefill_logits(int rows) {
    ...
    // NOTE: pre-existing nesting, preserved verbatim.  The softcap is applied
    // only when the logits scale is non-trivial.
