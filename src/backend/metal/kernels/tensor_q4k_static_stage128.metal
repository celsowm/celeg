// Strict Q4_K TensorOps fast path for fully aligned prefill tiles.
//
// Two production K64 slices are staged together and consumed by two strict
// K64 matmul2d runs in the same order as the baseline. Static tensor extents
// remove full-tile bounds checks. The host dispatches this kernel only when
// rows are divisible by 128, columns by 128, output rows by 64, and 16 KiB of
// threadgroup memory is available. The dynamic production kernel remains the
