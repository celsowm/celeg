...
/**
 * @brief Benchmark-only tile matching llama.cpp's current Metal Tensor shape.
 *
 * Production continues to use the 64x128 tile above. The larger 128-output-row
 * by 256-token tile keeps four simdgroups, unlike the earlier 128-row experiment
 * that scaled the cooperative width with the row tile. This isolates whether
 * fewer row/token tiles can reduce activation rereads without the occupancy
 * collapse seen in that experiment.
 */
