...
void update_online(const float* query, int query_head, int kv_head, int head_dim, float scale,
                   const CpuKvPagePool& pool, CpuKvPageId page, int token_begin, int token_end,
                   int page_token_base, int query_position, const CpuAttentionPattern& pattern,
                   const CpuAttentionBias& bias, float* accumulator, PartialAttention& state) {
#if CELEG_CPU_HAS_AVX2_KERNEL
    if (g_has_avx2_fma && bias.empty() && pattern.parallel_safe()) {
        update_online_avx2(query, kv_head, head_dim, scale, pool, page, token_begin, token_end, accumulator, state);
        return;
    }
#endif
    for (int local = token_begin; local < token_end; ++local) {
        ...
        float dot = 0.0f;
        if (pool.mode() == CpuKvCacheMode::Fp32) {
            const float* key = pool.key_fp32(page, static_cast<size_t>(local)) + static_cast<size_t>(kv_head) * head_dim;
            for (int d = 0; d < head_dim; ++d) dot += query[d] * key[d];
        } else {
            const uint16_t* key = pool.key_bf16(page, static_cast<size_t>(local)) + static_cast<size_t>(kv_head) * head_dim;
...
void cpu_gqa_decode_paged(const float* q, const CpuKvPagePool& pool, std::span<const CpuKvPageId> pages,
                          float* output, int sequence_length, int q_heads, int kv_heads, int head_dim,
                          CpuAttentionPattern pattern, CpuAttentionBias bias, int query_position) {
    if (!q || !output) throw std::invalid_argument("paged GQA pointers are required");
    if (sequence_length <= 0 || q_heads <= 0 || kv_heads <= 0 || head_dim <= 0 || q_heads % kv_heads != 0)
    ...
    for (int qh = 0; qh < q_heads; ++qh) {
        ...
        for (size_t page_index = first_page; page_index < required_pages; ++page_index) {
            ...
            const int tokens_in_page = std::min<int>(static_cast<int>(pool.page_tokens()) - offset, sequence_length - page_start - offset);
            update_online(query, qh, kvh, head_dim, scale, pool, pages[page_index], offset, offset + tokens_in_page,
                          page_start, query_position, pattern, bias, destination, state);
        }
        if (state.denominator == 0.0f) throw std::invalid_argument("attention pattern selected no KV tokens");
        const float reciprocal = 1.0f / state.denominator;
        for (int d = 0; d < head_dim; ++d) destination[d] *= reciprocal;
    }
}

void cpu_gqa_decode_paged_parallel(const float* q, const CpuKvPagePool& pool, std::span<const CpuKvPageId> pages,
                                   float* output, int sequence_length, int q_heads, int kv_heads, int head_dim,
                                   CpuThreadPool& thread_pool, CpuAttentionPattern pattern, CpuAttentionBias bias,
                                   CpuPagedAttentionOptions options, CpuPagedAttentionStats* stats) {
    ...
    if (!pattern.parallel_safe() || static_cast<size_t>(sequence_length) < options.parallel_threshold || thread_pool.size() == 0 || tiles == 0) {
        cpu_gqa_decode_paged(q, pool, pages, output, sequence_length, q_heads, kv_heads, head_dim, pattern, bias);
        if (stats) *stats = {static_cast<size_t>(q_heads), tiles, false};
        return;
    }
    if (!q || !output || q_heads <= 0 || kv_heads <= 0 || head_dim <= 0 || q_heads % kv_heads != 0 || pages.size() < required_pages)
...
void cpu_latent_attention_decode_paged(const float* query_content, const float* query_rope,
    const CpuKvPagePool& pool, std::span<const CpuKvPageId> pages, float* output,
    int sequence_length, int query_heads, int latent_rank, int rope_head_dim, float scale,
    CpuAttentionPattern pattern, CpuAttentionBias bias, int query_position) {
    if (!query_content || !output || sequence_length <= 0 || query_heads <= 0 || latent_rank <= 0 || rope_head_dim < 0 ||
    ...
    for (int qh = 0; qh < query_heads; ++qh) {
        ...
        for (int key_position = first_token; key_position < sequence_length; ++key_position) {
            ...
            if (pool.mode() == CpuKvCacheMode::Fp32) {
                ...
                if (rope_head_dim != 0) score += std::inner_product(rotary, rotary + rope_head_dim, pool.rotary_fp32(pages[page_index], offset), 0.0f);
            } else {
        ...
        for (int key_position = first_token; key_position < sequence_length; ++key_position) {
            ...
            const float probability = std::exp(score * scale + bias.score(qh, query_position, key_position) - maximum);
            ...
            if (pool.mode() == CpuKvCacheMode::Fp32) {
                ...
                for (int d = 0; d < latent_rank; ++d) destination[d] += probability * bf16_bits_to_float(value[d]);
            }
        }
