...
class NativeTagToolCallCodec final : public IChatToolCallCodec {
public:
    ...
    ToolParseResult parse_generation(std::string_view text) const override {
        ...
        if (!result.calls.empty()) {
            ...
            result.error = "tagged tool-call block contains no valid function call";
        }
        return result;
    }
...
std::unique_ptr<IChatToolCallCodec> make_native_tag_tool_codec() {
    return std::make_unique<NativeTagToolCallCodec>();
}
