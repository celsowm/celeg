...
std::string PatchRoleChatTemplate::format(
    std::span<const ChatMessage> messages,
    std::span<const ChatToolDefinition> tools,
    bool add_generation_prompt,
    const ChatTemplateOptions& options) const {
    chat_template_detail::reject_unhandled_options(options, "chat:patch-role");
    if (!tools.empty()) {
        throw std::invalid_argument(
            "patch-role tool formatting requires structured tool-call support");
    }
    ...
    for (const ChatMessage& message : messages) {
        if (message.role == ChatRole::Tool || message.role == ChatRole::Developer) {
            throw std::invalid_argument("patch-role profile does not support this chat role");
        }
        std::string content = message.content.value_or("");
        constexpr std::string_view generic_image_marker = "<|image|>";
        constexpr std::string_view patch_marker = "<|patch|>";
        for (std::size_t found = content.find(generic_image_marker);
             found != std::string::npos;
             found = content.find(generic_image_marker, found + patch_marker.size())) {
            content.replace(found, generic_image_marker.size(), patch_marker);
        }
...
void add_patch_role_chat_template(ChatTemplateCatalog& catalog) {
    ...
    catalog.add("chat:patch-role", std::make_unique<PatchRoleChatTemplate>(), nullptr,
                capabilities);
}
