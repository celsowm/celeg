...
const char* role_tag(ChatRole role) {
    switch (role) {
        ...
        case ChatRole::Tool:
            throw std::invalid_argument("RoleEnvelopeChatTemplate does not support tool messages");
    }
...
std::string RoleEnvelopeChatTemplate::format(
    std::span<const ChatMessage> messages,
    std::span<const ChatToolDefinition> tools,
    bool add_generation_prompt,
    const ChatTemplateOptions& options) const {
    chat_template_detail::reject_unhandled_options(options, "chat:role-envelope");
...
void add_role_envelope_chat_template(ChatTemplateCatalog& catalog) {
    catalog.add("chat:role-envelope", std::make_unique<RoleEnvelopeChatTemplate>());
}
