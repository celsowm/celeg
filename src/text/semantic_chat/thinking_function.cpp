...
std::string ThinkingFunctionChatTemplate::format(
    std::span<const ChatMessage> messages,
    std::span<const ChatToolDefinition> tools,
    bool add_generation_prompt,
    const ChatTemplateOptions& options) const {
    chat_template_detail::reject_unhandled_options(options, "chat:thinking-function");
    std::string out = "<bos>";
...
void add_thinking_function_chat_template(ChatTemplateCatalog& catalog) {
    ...
    catalog.add("chat:thinking-function", std::make_unique<ThinkingFunctionChatTemplate>(),
                make_function_xml_tool_codec(), capabilities);
}
