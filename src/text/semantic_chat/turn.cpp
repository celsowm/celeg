...
std::string TurnChatTemplate::format(
    std::span<const ChatMessage> messages,
    std::span<const ChatToolDefinition> tools,
    bool add_generation_prompt,
    const ChatTemplateOptions& options) const {
    chat_template_detail::reject_unhandled_options(options, "chat:turn");
    std::string out = "<bos>";
    if (!tools.empty()) {
        out += "<|turn>system\n";
...
void add_turn_chat_template(ChatTemplateCatalog& catalog) {
    ...
    catalog.add("chat:turn", std::make_unique<TurnChatTemplate>(),
                make_native_tag_tool_codec(), capabilities);
}
