...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::filesystem::path config_path = config_path_for(model_path);
        if (!std::filesystem::exists(config_path)) {
            throw std::runtime_error("config.json not found alongside checkpoint: " + config_path.string());
        }

        lfm::ModelConfig config = lfm::ModelConfig::load(config_path.string());
        lfm::ModelShape shape = lfm::ModelShape::from_config(config);
