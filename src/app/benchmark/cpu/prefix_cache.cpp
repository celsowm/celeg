...
int main(int argc, char** argv) {
    try {
        if (argc < 3) {
            std::cerr << "usage: celeg-cpu-prefix-cache-benchmark MODEL_DIR PROMPT [REPEATS] [SUFFIX]\n";
            return 2;
        }
        ...
        const auto chat_template = celeg::resolve_interaction(
        ...
        model_options.numa_mode = celeg::CpuNumaMode::Local;
