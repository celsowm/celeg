...
struct Tensor {
    ...
    std::size_t size() const {
        ...
        for (const std::int64_t dimension : view.shape) {
            if (dimension <= 0) throw std::invalid_argument("Muse vision tensor has invalid shape");
    ...
    float value(std::size_t index) const {
        if (index >= size()) throw std::out_of_range("Muse vision tensor index out of range");
        ...
        throw std::invalid_argument("Muse vision weights must be F32, BF16, or F16");
    }
};
...
void require_shape(const Tensor& tensor, std::initializer_list<std::int64_t> expected,
    ...
    if (tensor.view.shape.size() != expected.size() ||
        ...
        throw std::invalid_argument("Muse vision tensor has unexpected shape: " + std::string(name));
    }
}
...
class MuseGlimmerVisionProvider final : public IVisualEmbeddingProvider {
public:
    explicit MuseGlimmerVisionProvider(std::shared_ptr<const IWeightRepository> repository)
...
private:
    ...
    Tensor tensor(std::string_view name) const {
        if (!repository_->contains(name)) {
            throw std::invalid_argument("missing Muse vision tensor: " + std::string(name));
        }
...
class MuseGlimmerVisionProviderFactory final : public IVisionProviderFactory {
public:
    std::string_view id() const override { return "muse-glimmer"; }
    bool supports(std::string_view architecture_id, const std::filesystem::path& model_path) const override {
        return architecture_id == "muse-glimmer" && std::filesystem::is_directory(model_path) &&
    ...
    std::shared_ptr<const IVisualEmbeddingProvider> create(const std::filesystem::path& model_path) const override {
        return make_muse_glimmer_visual_embedding_provider(model_path);
    }
};

} // namespace

VisualEmbeddingProvider make_muse_glimmer_visual_embedding_provider(const std::filesystem::path& model_path) {
    return make_muse_glimmer_visual_embedding_provider(
        std::make_shared<SafeTensorRepository>(model_path));
}

VisualEmbeddingProvider make_muse_glimmer_visual_embedding_provider(
    std::shared_ptr<const IWeightRepository> repository) {
    if (!repository) throw std::invalid_argument("Muse Glimmer vision repository is null");
    return std::make_shared<MuseGlimmerVisionProvider>(std::move(repository));
}

std::unique_ptr<IVisionProviderFactory> make_muse_glimmer_vision_provider_factory() {
    return std::make_unique<MuseGlimmerVisionProviderFactory>();
}
