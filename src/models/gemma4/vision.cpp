...
struct Tensor {
    ...
    float value(size_t index) const {
        ...
        throw std::invalid_argument("Gemma visual projector requires F32 or BF16 tensors");
    }
...
void rmsnorm(std::vector<float>& values, const Tensor& weight, float epsilon,
    ...
    if (weight.elements() != width || values.size() % width != 0) {
        throw std::invalid_argument("Gemma visual norm shape mismatch");
    }
...
void rmsnorm_heads(std::vector<float>& values, const Tensor& weight,
    ...
    if (weight.elements() != static_cast<size_t>(head_width)) {
        throw std::invalid_argument("Gemma visual head norm shape mismatch");
    }
...
std::vector<float> linear(const Tensor& weight, const std::vector<float>& input,
    ...
    if (weight.elements() != rows * cols || input.size() % cols != 0) {
        throw std::invalid_argument("Gemma visual linear shape mismatch");
    }
...
class Gemma4VisionProvider final : public IVisualEmbeddingProvider {
public:
    explicit Gemma4VisionProvider(const std::filesystem::path& path)
        ...
        if (file_->str_or("clip.vision.projector_type", "") != "gemma4v") {
            throw std::invalid_argument("visual projector is not a Gemma4V projector");
        }
        ...
        if (image_size_ <= 0 || patch_size_ <= 0 || hidden_ <= 0 || heads_ <= 0 ||
            ...
            throw std::invalid_argument("invalid Gemma4V projector metadata");
        }
        ...
        if (position_stride_ < image_size_ / patch_size_ ||
            ...
            throw std::invalid_argument("Gemma4V projector shape metadata mismatch");
        }
    }
...
VisualEmbeddingProvider make_gemma4_visual_embedding_provider(
    ...
    return std::make_shared<Gemma4VisionProvider>(projector_path);
}
