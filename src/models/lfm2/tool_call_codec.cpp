...
class Lfm2ToolCallCodec final : public IChatToolCallCodec {
public:
    ...
    ToolParseResult parse_generation(std::string_view text) const override {
        ...
        if (!result.calls.empty()) {
            ...
            result.error = "LFM2 tool-call block contains no valid function call";
        }
        return result;
    }
...
std::unique_ptr<IChatToolCallCodec> make_lfm2_tool_call_codec() {
    return std::make_unique<Lfm2ToolCallCodec>();
}
