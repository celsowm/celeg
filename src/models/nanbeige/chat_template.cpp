...
const char* role_name(ChatRole role) {
    ...
    throw std::invalid_argument("unknown Nanbeige chat role");
}

} // namespace

std::string Nanbeige42InstructChatTemplate::format(
    ...
    if (!messages.empty() && messages.front().role == ChatRole::System) {
        ...
        if (!tools.empty()) {
            out += "\n" + make_nanbeige42_tool_call_codec()->render_tool_definitions(tools, {});
        }
        out += "<|im_end|>\n";
        wrote_system = true;
    } else if (!tools.empty()) {
        out += "<|im_start|>system\n" + std::string(kDefaultSystem) + "\n";
        out += make_nanbeige42_tool_call_codec()->render_tool_definitions(tools, {});
        out += "<|im_end|>\n";
        wrote_system = true;
    ...
    const auto codec = make_nanbeige42_tool_call_codec();
...
void add_nanbeige42_chat_profile(ChatProfileCatalog& catalog) {
    ...
    catalog.add("nanbeige42-instruct", std::make_unique<Nanbeige42InstructChatTemplate>(),
                make_nanbeige42_tool_call_codec(), capabilities);
}
