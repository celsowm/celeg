...
lfm25_cpu_model* create_model_impl(
    const char* path,
    int32_t max_context,
    int32_t threads,
    int32_t isa,
    ...
    try {
        ...
        options.isa = convert_isa(isa);
        options.threads = threads > 0 ? static_cast<size_t>(threads) : 0;
        options.affinity = convert_affinity(affinity);
        options.kv_cache_mode = convert_kv_mode(kv_cache_mode);
        options.kv_page_tokens = kv_page_tokens;
        options.prefill_chunk_tokens = prefill_chunk_tokens;
        options.prefill_chunk_threshold = prefill_chunk_threshold;
        options.attention_parallel_threshold = attention_parallel_threshold;
        options.attention_page_tile = attention_page_tile;
        options.numa_mode = convert_numa_mode(numa_mode);
        options.weight_format = q4_group_size == 64
            ? lfm::CpuWeightFormat::Q4Group64 : lfm::CpuWeightFormat::Q4Group32;
        if (q4_group_size != 32 && q4_group_size != 64) {
            throw std::invalid_argument("CPU Q4 group size must be 32 or 64");
        }
        options.use_pack_cache = use_pack_cache != 0;
        if (pack_cache_directory) options.pack_cache_directory = pack_cache_directory;
...
void lfm25_cpu_model_options_init(lfm25_cpu_model_options_v1* options) {
    if (!options) return;
    *options = {};
...
void lfm25_cpu_model_options_v2_init(lfm25_cpu_model_options_v2* options) {
    if (!options) return;
    *options = {};
    options->struct_size = sizeof(*options);
    options->max_context = 4096;
...
void lfm25_cpu_model_options_v3_init(lfm25_cpu_model_options_v3* options) {
    if (!options) return;
    *options = {};
    options->struct_size = sizeof(*options);
    options->max_context = 4096;
...
void lfm25_cpu_model_options_v4_init(lfm25_cpu_model_options_v4* options) {
    if (!options) return;
    *options = {};
    options->struct_size = sizeof(*options);
    options->max_context = 4096;
...
void lfm25_cpu_model_options_v5_init(lfm25_cpu_model_options_v5* options) {
    if (!options) return;
    *options = {};
    options->struct_size = sizeof(*options);
    options->max_context = 4096;
...
void lfm25_cpu_engine_options_init(lfm25_cpu_engine_options_v1* options) {
    if (!options) return;
    *options = {};
    options->struct_size = sizeof(*options);
...
void lfm25_cpu_engine_options_v2_init(lfm25_cpu_engine_options_v2* options) {
    if (!options) return;
    *options = {};
    options->struct_size = sizeof(*options);
    options->max_active_requests = 16;
...
void lfm25_cpu_engine_options_v3_init(lfm25_cpu_engine_options_v3* options) {
    if (!options) return;
    *options = {};
    options->struct_size = sizeof(*options);
    options->max_active_requests = 16;
...
lfm25_cpu_model* lfm25_cpu_model_create(
    const char* path, const lfm25_cpu_model_options_v1* input_options,
    const lfm25_cpu_generation_options_v1* input_generation) {
    lfm25_cpu_model_options_v1 defaults;
    lfm25_cpu_model_options_init(&defaults);
    const auto& source = input_options ? *input_options : defaults;
    if (input_options && input_options->struct_size < sizeof(lfm25_cpu_model_options_v1)) {
        global_text = "CPU options struct is too small";
        return nullptr;
    }
    return create_model_impl(path, source.max_context, source.threads,
        source.isa, source.q4_group_size, source.use_pack_cache,
        source.pack_cache_directory, LFM25_CPU_AFFINITY_NONE,
        LFM25_CPU_KV_FP32, 32, 256, 16, 256, 4, LFM25_CPU_NUMA_DISABLED, input_generation);
}
lfm25_cpu_model* lfm25_cpu_model_create_v2(
    const char* path, const lfm25_cpu_model_options_v2* input_options,
    const lfm25_cpu_generation_options_v1* input_generation) {
    lfm25_cpu_model_options_v2 defaults;
    lfm25_cpu_model_options_v2_init(&defaults);
    const auto& source = input_options ? *input_options : defaults;
    if (input_options && input_options->struct_size < sizeof(lfm25_cpu_model_options_v2)) {
        global_text = "CPU v2 options struct is too small";
        return nullptr;
    }
    return create_model_impl(path, source.max_context, source.threads,
        source.isa, source.q4_group_size, source.use_pack_cache,
        source.pack_cache_directory, source.affinity, LFM25_CPU_KV_FP32,
        32, 256, 16, 256, 4, LFM25_CPU_NUMA_DISABLED, input_generation);
}
lfm25_cpu_model* lfm25_cpu_model_create_v3(
    const char* path, const lfm25_cpu_model_options_v3* input_options,
    const lfm25_cpu_generation_options_v1* input_generation) {
    lfm25_cpu_model_options_v3 defaults;
    lfm25_cpu_model_options_v3_init(&defaults);
    const auto& source = input_options ? *input_options : defaults;
    if (input_options && input_options->struct_size < sizeof(lfm25_cpu_model_options_v3)) {
        global_text = "CPU v3 options struct is too small";
        return nullptr;
    }
    return create_model_impl(path, source.max_context, source.threads,
        source.isa, source.q4_group_size, source.use_pack_cache,
        source.pack_cache_directory, source.affinity, source.kv_cache_mode,
        32, 256, 16, 256, 4, LFM25_CPU_NUMA_DISABLED, input_generation);
}
lfm25_cpu_model* lfm25_cpu_model_create_v4(
    const char* path, const lfm25_cpu_model_options_v4* input_options,
    const lfm25_cpu_generation_options_v1* input_generation) {
    lfm25_cpu_model_options_v4 defaults;
    lfm25_cpu_model_options_v4_init(&defaults);
    const auto& source = input_options ? *input_options : defaults;
    if (input_options && input_options->struct_size < sizeof(lfm25_cpu_model_options_v4)) {
        global_text = "CPU v4 options struct is too small";
        return nullptr;
    }
    return create_model_impl(path, source.max_context, source.threads,
        source.isa, source.q4_group_size, source.use_pack_cache,
        source.pack_cache_directory, source.affinity, source.kv_cache_mode,
        source.kv_page_tokens, source.prefill_chunk_tokens,
        source.prefill_chunk_threshold, 256, 4, LFM25_CPU_NUMA_DISABLED, input_generation);
}
lfm25_cpu_model* lfm25_cpu_model_create_v5(
    const char* path, const lfm25_cpu_model_options_v5* input_options,
    const lfm25_cpu_generation_options_v1* input_generation) {
    lfm25_cpu_model_options_v5 defaults;
    lfm25_cpu_model_options_v5_init(&defaults);
    const auto& source = input_options ? *input_options : defaults;
    if (input_options && input_options->struct_size < sizeof(lfm25_cpu_model_options_v5)) {
        global_text = "CPU v5 options struct is too small";
        return nullptr;
    }
    return create_model_impl(path, source.max_context, source.threads,
        source.isa, source.q4_group_size, source.use_pack_cache,
        source.pack_cache_directory, source.affinity, source.kv_cache_mode,
        source.kv_page_tokens, source.prefill_chunk_tokens,
        source.prefill_chunk_threshold, source.attention_parallel_threshold,
        source.attention_page_tile, source.numa_mode, input_generation);
}
...
lfm25_cpu_status lfm25_cpu_model_get_memory_stats_v2(
    lfm25_cpu_model* model, lfm25_cpu_memory_stats_v2* out) {
    if (!out || out->struct_size < sizeof(*out)) return LFM25_CPU_STATUS_INVALID_ARGUMENT;
    return protect(model, [&] {
        const auto s = model->value->memory_stats();
        out->weights = s.weights; out->kv_cache = s.kv_cache;
...
lfm25_cpu_engine* lfm25_cpu_engine_create(
    const char* path, const lfm25_cpu_model_options_v3* input_model,
    const lfm25_cpu_engine_options_v1* input_engine) {
    auto handle = std::make_unique<lfm25_cpu_engine>();
    try {
        if (!path || !*path) throw std::invalid_argument("safetensors_path is required");
        lfm25_cpu_model_options_v3 model_defaults;
        lfm25_cpu_model_options_v3_init(&model_defaults);
        lfm25_cpu_engine_options_v1 engine_defaults;
        ...
        if (input_model && input_model->struct_size < sizeof(lfm25_cpu_model_options_v3)) {
            throw std::invalid_argument("CPU v3 options struct is too small");
        }
        if (input_engine && input_engine->struct_size < sizeof(lfm25_cpu_engine_options_v1)) {
            throw std::invalid_argument("CPU engine options struct is too small");
        }
        lfm::CpuModelOptions model_options;
        model_options.isa = convert_isa(model_source.isa);
        model_options.threads = model_source.threads > 0 ?
            static_cast<size_t>(model_source.threads) : 0;
        model_options.affinity = convert_affinity(model_source.affinity);
        model_options.kv_cache_mode = convert_kv_mode(model_source.kv_cache_mode);
        model_options.weight_format = model_source.q4_group_size == 64 ?
            lfm::CpuWeightFormat::Q4Group64 : lfm::CpuWeightFormat::Q4Group32;
        if (model_source.q4_group_size != 32 && model_source.q4_group_size != 64) {
            throw std::invalid_argument("CPU Q4 group size must be 32 or 64");
        }
        model_options.use_pack_cache = model_source.use_pack_cache != 0;
        if (model_source.pack_cache_directory) {
            model_options.pack_cache_directory = model_source.pack_cache_directory;
        }
...
lfm25_cpu_engine* lfm25_cpu_engine_create_v2(
    const char* path, const lfm25_cpu_model_options_v4* input_model,
    const lfm25_cpu_engine_options_v2* input_engine) {
    auto handle = std::make_unique<lfm25_cpu_engine>();
    try {
        if (!path || !*path) throw std::invalid_argument("safetensors_path is required");
        lfm25_cpu_model_options_v4 model_defaults;
        lfm25_cpu_model_options_v4_init(&model_defaults);
        lfm25_cpu_engine_options_v2 engine_defaults;
        lfm25_cpu_engine_options_v2_init(&engine_defaults);
        const auto& model_source = input_model ? *input_model : model_defaults;
        const auto& engine_source = input_engine ? *input_engine : engine_defaults;
        if (input_model && input_model->struct_size < sizeof(lfm25_cpu_model_options_v4)) {
            throw std::invalid_argument("CPU v4 options struct is too small");
        }
        if (input_engine && input_engine->struct_size < sizeof(lfm25_cpu_engine_options_v2)) {
            throw std::invalid_argument("CPU engine v2 options struct is too small");
        }
        lfm::CpuModelOptions model_options;
        model_options.isa = convert_isa(model_source.isa);
        model_options.threads = model_source.threads > 0 ?
        ...
        model_options.affinity = convert_affinity(model_source.affinity);
        model_options.kv_cache_mode = convert_kv_mode(model_source.kv_cache_mode);
        model_options.kv_page_tokens = model_source.kv_page_tokens;
        model_options.prefill_chunk_tokens = model_source.prefill_chunk_tokens;
        model_options.prefill_chunk_threshold = model_source.prefill_chunk_threshold;
        model_options.weight_format = model_source.q4_group_size == 64 ?
            lfm::CpuWeightFormat::Q4Group64 : lfm::CpuWeightFormat::Q4Group32;
        if (model_source.q4_group_size != 32 && model_source.q4_group_size != 64) {
            throw std::invalid_argument("CPU Q4 group size must be 32 or 64");
...
lfm25_cpu_engine* lfm25_cpu_engine_create_v3(
    const char* path, const lfm25_cpu_model_options_v5* input_model,
    const lfm25_cpu_engine_options_v3* input_engine) {
    auto handle = std::make_unique<lfm25_cpu_engine>();
    try {
        if (!path || !*path) throw std::invalid_argument("safetensors_path is required");
        lfm25_cpu_model_options_v5 model_defaults;
        lfm25_cpu_model_options_v5_init(&model_defaults);
        lfm25_cpu_engine_options_v3 engine_defaults;
        lfm25_cpu_engine_options_v3_init(&engine_defaults);
        const auto& model_source = input_model ? *input_model : model_defaults;
        const auto& engine_source = input_engine ? *input_engine : engine_defaults;
        if (input_model && input_model->struct_size < sizeof(lfm25_cpu_model_options_v5)) {
            throw std::invalid_argument("CPU v5 options struct is too small");
        }
        if (input_engine && input_engine->struct_size < sizeof(lfm25_cpu_engine_options_v3)) {
            throw std::invalid_argument("CPU engine v3 options struct is too small");
        }
        lfm::CpuModelOptions model_options;
        model_options.isa = convert_isa(model_source.isa);
        model_options.threads = model_source.threads > 0 ?
        ...
        model_options.prefill_chunk_tokens = model_source.prefill_chunk_tokens;
        model_options.prefill_chunk_threshold = model_source.prefill_chunk_threshold;
        model_options.attention_parallel_threshold = model_source.attention_parallel_threshold;
        model_options.attention_page_tile = model_source.attention_page_tile;
        model_options.numa_mode = convert_numa_mode(model_source.numa_mode);
        model_options.weight_format = model_source.q4_group_size == 64 ?
            lfm::CpuWeightFormat::Q4Group64 : lfm::CpuWeightFormat::Q4Group32;
        if (model_source.q4_group_size != 32 && model_source.q4_group_size != 64) {
            throw std::invalid_argument("CPU Q4 group size must be 32 or 64");
...
lfm25_cpu_status lfm25_cpu_engine_get_metrics(
    lfm25_cpu_engine* engine, lfm25_cpu_engine_metrics_v1* out) {
    if (!engine || !out || out->struct_size < sizeof(*out)) {
        return LFM25_CPU_STATUS_INVALID_ARGUMENT;
    }
    return protect(engine, [&] {
        const lfm::CpuConcurrentMetrics metrics = engine->value->metrics();
        out->submitted_requests = metrics.submitted_requests;
        out->completed_requests = metrics.completed_requests;
        out->cancelled_requests = metrics.cancelled_requests;
        out->failed_requests = metrics.failed_requests;
        out->active_requests = metrics.active_requests;
        out->queued_requests = metrics.queued_requests;
        out->prefill_tokens = metrics.prefill_tokens;
        out->decode_tokens = metrics.decode_tokens;
        out->ragged_prefill_steps = metrics.ragged_prefill_steps;
        out->packed_decode_steps = metrics.packed_decode_steps;
        out->maximum_prefill_batch = metrics.maximum_prefill_batch;
        out->maximum_decode_batch = metrics.maximum_decode_batch;
        ...
        out->average_ttft_ms = metrics.average_ttft_ms();
        out->average_itl_ms = metrics.average_itl_ms();
        out->scheduler_ms = metrics.cumulative_scheduler_ms;
    });
}
lfm25_cpu_status lfm25_cpu_engine_get_metrics_v2(
    lfm25_cpu_engine* engine, lfm25_cpu_engine_metrics_v2* out) {
    if (!engine || !out || out->struct_size < sizeof(*out)) {
        return LFM25_CPU_STATUS_INVALID_ARGUMENT;
    }
    return protect(engine, [&] {
        ...
        out->average_itl_ms = metrics.average_itl_ms();
        out->scheduler_ms = metrics.cumulative_scheduler_ms;
        out->chunked_prefill_steps = metrics.chunked_prefill_steps;
        out->chunked_prefill_tokens = metrics.chunked_prefill_tokens;
        out->maximum_prefill_chunk = metrics.maximum_prefill_chunk;
    });
}
lfm25_cpu_status lfm25_cpu_engine_get_metrics_v3(
    ...
    return protect(engine, [&] {
        ...
        out->base.struct_size = sizeof(out->base);
        out->base.submitted_requests = m.submitted_requests;
        out->base.completed_requests = m.completed_requests;
        out->base.cancelled_requests = m.cancelled_requests;
        out->base.failed_requests = m.failed_requests;
        out->base.active_requests = m.active_requests;
        out->base.queued_requests = m.queued_requests;
        out->base.prefill_tokens = m.prefill_tokens;
        out->base.decode_tokens = m.decode_tokens;
        out->base.ragged_prefill_steps = m.ragged_prefill_steps;
        out->base.packed_decode_steps = m.packed_decode_steps;
        out->base.maximum_prefill_batch = m.maximum_prefill_batch;
        out->base.maximum_decode_batch = m.maximum_decode_batch;
        out->base.prefill_tokens_per_second = m.prefill_tokens_per_second();
        out->base.decode_tokens_per_second = m.decode_tokens_per_second();
        out->base.average_ttft_ms = m.average_ttft_ms();
        out->base.average_itl_ms = m.average_itl_ms();
        out->base.scheduler_ms = m.cumulative_scheduler_ms;
        out->base.chunked_prefill_steps = m.chunked_prefill_steps;
        out->base.chunked_prefill_tokens = m.chunked_prefill_tokens;
        out->base.maximum_prefill_chunk = m.maximum_prefill_chunk;
