...
constexpr uint32_t kRotaryDim = 6;
constexpr float kTolerance = 5.0e-5f;
constexpr std::array<uint32_t, 3> kMetalSections{1, 1, 1};
constexpr std::array<int, 3> kCpuSections{1, 1, 1};

celeg::RopePositionSpec rope_spec(float theta) {
...
id<MTLBuffer> float_buffer(id<MTLDevice> device, const std::vector<float>& values) {
    ...
    if (!result) throw std::runtime_error("Metal partial MRoPE buffer allocation failed");
    return result;
}
...
id<MTLBuffer> int_buffer(id<MTLDevice> device, const std::vector<int32_t>& values) {
    id<MTLBuffer> result = [device newBufferWithBytes:values.data()
                                                ...
                                                options:MTLResourceStorageModeShared];
    if (!result) throw std::runtime_error("Metal partial MRoPE position buffer failed");
    return result;
}
...
id<MTLComputePipelineState> pipeline(id<MTLDevice> device, id<MTLLibrary> library,
    ...
    if (!function) throw std::runtime_error(std::string("missing Metal partial MRoPE kernel: ") + name);
    id<MTLComputePipelineState> result =
        [device newComputePipelineStateWithFunction:function error:&error];
    if (!result) {
        throw std::runtime_error("Metal partial MRoPE pipeline failed: " +
...
void check_tail_passthrough(const float* actual, const float* input,
                            float scale, const char* label) {
    for (uint32_t d = kRotaryDim; d < kHeadDim; ++d) {
        const float expected = input[d] * scale;
        const float delta = std::abs(actual[d] - expected);
        if (delta > 1.0e-6f) {
            throw std::runtime_error(std::string(label) + " rotated the pass-through tail at dim " +
                std::to_string(d) + ": actual=" + std::to_string(actual[d]) +
...
void cpu_mrope(float* values,
               ...
               float scale,
               float theta) {
    const celeg::RopePositionSpec rope = rope_spec(theta);
...
void run_position_store_case(id<MTLDevice> device, id<MTLLibrary> library,
    ...
    cpu_mrope(expected_query.data(), position, is_interleaved, query_scale, theta);
    cpu_mrope(expected_key.data(), position, is_interleaved, 1.0f, theta);
    ...
    [encoder setBytes:&theta_value length:sizeof(theta_value) atIndex:11];
    ...
    [encoder endEncoding];
    wait(command, "Metal partial MRoPE position-store");

    check_close(static_cast<const float*>(query.contents), expected_query, "partial position query");
    check_close(static_cast<const float*>(key.contents), expected_key, "partial position key");
    check_tail_passthrough(static_cast<const float*>(query.contents), query_values.data(),
                           query_scale, "partial position query");
    check_tail_passthrough(static_cast<const float*>(key.contents), key_values.data(),
                           1.0f, "partial position key");
    ...
    const auto* value_cache_values = static_cast<const float*>(value_cache.contents) +
        static_cast<size_t>(cache_position) * kHeadDim;
    check_close(key_cache_values, expected_key, "partial position key cache");
    check_close(value_cache_values, value_values, "partial position value cache", 0.0f);
}
...
void run_fused_norm_case(id<MTLDevice> device, id<MTLLibrary> library,
    ...
    cpu_norm_mrope(expected_query.data(), query_weight, query_epsilon,
                   position, is_interleaved, query_scale, theta);
    cpu_norm_mrope(expected_key.data(), key_weight, key_epsilon,
                   position, is_interleaved, 1.0f, theta);
    ...
    [encoder setBytes:&theta_value length:sizeof(theta_value) atIndex:13];
    ...
    [encoder dispatchThreads:MTLSizeMake(1, 1, 1)
       threadsPerThreadgroup:MTLSizeMake(1, 1, 1)];
    [encoder endEncoding];
    wait(command, "Metal partial fused norm MRoPE");

    check_close(static_cast<const float*>(query.contents), expected_query, "partial fused query", 2.0e-4f);
    check_close(static_cast<const float*>(key.contents), expected_key, "partial fused key", 2.0e-4f);
    const auto* key_cache_values = static_cast<const float*>(key_cache.contents) +
        static_cast<size_t>(cache_position) * kHeadDim;
    const auto* value_cache_values = static_cast<const float*>(value_cache.contents) +
        static_cast<size_t>(cache_position) * kHeadDim;
    check_close(key_cache_values, expected_key, "partial fused key cache", 2.0e-4f);
    check_close(value_cache_values, value_values, "partial fused value cache", 0.0f);
}
...
void run_batch_case(id<MTLDevice> device, id<MTLLibrary> library,
    ...
    for (uint32_t row = 0; row < rows; ++row) {
        ...
        cpu_mrope(expected_query_row, positions[row], is_interleaved, query_scale, theta);
        cpu_mrope(expected_key_row, positions[row], is_interleaved, 1.0f, theta);
    }
    ...
    [encoder setBytes:&theta_value length:sizeof(theta_value) atIndex:8];
    ...
    [encoder endEncoding];
    wait(command, "Metal partial batched MRoPE");

    check_close(static_cast<const float*>(query.contents), expected_query, "partial batch query");
    check_close(static_cast<const float*>(key.contents), expected_key, "partial batch key");
    const auto* query_out = static_cast<const float*>(query.contents);
    const auto* key_out = static_cast<const float*>(key.contents);
    for (uint32_t row = 0; row < rows; ++row) {
        check_tail_passthrough(query_out + static_cast<size_t>(row) * kHeadDim,
                               query_values.data() + static_cast<size_t>(row) * kHeadDim,
                               query_scale, "partial batch query");
        check_tail_passthrough(key_out + static_cast<size_t>(row) * kHeadDim,
                               key_values.data() + static_cast<size_t>(row) * kHeadDim,
                               1.0f, "partial batch key");
    }
}

void prove_partial_differs_from_full_width() {
    const std::array<int32_t, 3> position{2, 7, 13};
    std::vector<float> partial = initial_values(0.31f, kHeadDim);
    std::vector<float> full = partial;
    celeg::RopePositionSpec partial_rope = rope_spec(10000.0f);
    celeg::RopePositionSpec full_rope = rope_spec(10000.0f);
    full_rope.rotary_fraction = 1.0;
    celeg::cpu_rope_mrope(partial.data(), 1, static_cast<int>(kHeadDim), position,
                          kCpuSections, true, partial_rope);
    celeg::cpu_rope_mrope(full.data(), 1, static_cast<int>(kHeadDim), position,
                          {4, 1, 1}, true, full_rope);
    bool differs = false;
    for (size_t i = 0; i < partial.size(); ++i) {
        differs = differs || std::abs(partial[i] - full[i]) > 1.0e-4f;
    }
    if (!differs) throw std::runtime_error("partial MRoPE vectors do not differ from full width");
}
...
int main() {
    try {
        prove_partial_differs_from_full_width();
        ...
        if (!library) {
            throw std::runtime_error("Metal partial MRoPE shader compilation failed: " +
                (error ? ns_string(error.localizedDescription) : "unknown error"));
        }
        id<MTLCommandQueue> queue = [device newCommandQueue];
        if (!queue) throw std::runtime_error("Metal partial MRoPE command queue failed");

        for (float theta : std::array<float, 2>{10000.0f, 10000000.0f}) {
            for (bool interleaved : std::array<bool, 2>{true, false}) {
                run_position_store_case(device, library, queue, interleaved, theta);
                run_fused_norm_case(device, library, queue, interleaved, theta);
                run_batch_case(device, library, queue, interleaved, theta);
            }
        }

        std::cout << "metal partial MRoPE CPU parity passed\n";
        return 0;
