...
celeg::ResolvedModel dynamic_sparse_model() {
    ...
    model.provenance.identity = "cpu-dynamic-sparse-rejection";
...
bool compiler_rejects_dynamic_sparse() {
    try {
...
bool low_level_rejects_dynamic_sparse() {
    try {
...
int main() {
    CELEG_TEST_CHECK(compiler_rejects_dynamic_sparse());
    CELEG_TEST_CHECK(low_level_rejects_dynamic_sparse());
    return 0;
}
