...
int main() {
    try {
        const auto first = celeg::attention_semantics::merge_pair(
        ...
        require_close(
            celeg::attention_semantics::merge_accumulate(0.0f, 5.0f, first),
            5.0f);

        const auto second = celeg::attention_semantics::merge_pair(
            2.0f, 3.0f, 4.0f, 2.0f);
        ...
        require_close(
            celeg::attention_semantics::merge_accumulate(5.0f, 7.0f, second),
            5.0f * expected_old + 7.0f);

        require_close(
            celeg::attention_semantics::partial_rescale(1.5f, 4.0f),
            std::exp(-2.5f));
        return 0;
