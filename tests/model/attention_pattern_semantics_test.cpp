...
void causal_and_sliding() {
    ...
    CELEG_TEST_CHECK(causal_visible(8, 8));
    CELEG_TEST_CHECK(causal_visible(8, 0));
    CELEG_TEST_CHECK(!causal_visible(8, 9));
    CELEG_TEST_CHECK(sliding_window_first_candidate(8, 4) == 5);
    CELEG_TEST_CHECK(sliding_window_visible(8, 5, 4));
    CELEG_TEST_CHECK(!sliding_window_visible(8, 4, 4));
    CELEG_TEST_CHECK(!sliding_window_visible(8, 9, 4));
}

void prefix_semantics() {
    using namespace celeg::attention_semantics;
    CELEG_TEST_CHECK(prefix_lm_visible(2, 5, 6));
    CELEG_TEST_CHECK(!prefix_lm_visible(2, 6, 6));
    CELEG_TEST_CHECK(prefix_lm_visible(8, 6, 6));
    CELEG_TEST_CHECK(!prefix_lm_visible(8, 9, 6));
    CELEG_TEST_CHECK(prefix_lm_may_read_future(2, 10, 6));
    CELEG_TEST_CHECK(!prefix_lm_may_read_future(8, 10, 6));
}

void sparse_semantics() {
    using namespace celeg::attention_semantics;
    CELEG_TEST_CHECK(block_sparse_visible(63, 0, 16, 2, 1));
...
void cpu_matches_canonical() {
    ...
    CELEG_TEST_CHECK(causal.allows(8, 7) ==
        celeg::attention_semantics::causal_visible(8, 7));
    CELEG_TEST_CHECK(causal.allows(8, 9) ==
        celeg::attention_semantics::causal_visible(8, 9));
    ...
    for (int key = 0; key < 12; ++key) {
        CELEG_TEST_CHECK(sliding.allows(8, key) ==
            celeg::attention_semantics::sliding_window_visible(8, key, 4));
    }
    ...
    for (int key = 0; key < 80; ++key) {
        CELEG_TEST_CHECK(block.allows(63, key) ==
            celeg::attention_semantics::block_sparse_visible(63, key, 16, 2, 1));
    }
}
