...
int main() {
    ...
    const auto pack = celeg::quantize_float_groupwise_q4(values.data(), rows, cols, 32);
    pack.validate();
    ...
    const auto q8 = celeg::quantize_float_groupwise_q8(values.data(), cols, 32);
    q8.validate();
    ...
    for (size_t i = 0; i < bf16.size(); ++i) bf16[i] = celeg::float_to_bf16_bits(static_cast<float>(i) / 3.0f);
    const auto path = std::filesystem::temp_directory_path() / "celeg-cpu-pack-test.bin";
    {
    ...
    }
    {
        ...
        for (size_t i = 0; i < vector.size(); ++i) CELEG_TEST_CHECK(std::abs(vector[i] - celeg::bf16_bits_to_float(bf16[i])) < 1e-6f);
    }
