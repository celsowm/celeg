...
std::array<float, 6> cpu_reference(
    ...
    const celeg::RelativePositionBiasSpec spec{
        kBucketCount, kMaxDistance, bidirectional};
    const celeg::CpuAttentionBias bias = celeg::CpuAttentionBias::lower(
        spec, bias_values, 1);

    std::array<float, 6> output{};
    for (int row = 0; row < kRows; ++row) {
        const int visible_tokens = bidirectional ? kRows : row + 1;
...
void run_bf16(celeg::CudaStream& stream,
    ...
    const celeg::GqaContiguousArgs args{
        .query = dquery.data(),
        .kv = {.keys = dkeys.data(), .values = dvalues.data()},
        .out = output.data(),
        .geometry = {.q_heads = 1, .kv_heads = 1, .head_dim = kHeadDim},
        .extent = {.rows = kRows, .seq_len = bidirectional ? kRows : 0},
        .relative_bias = make_bias_view(bias, bidirectional),
        .stream = stream.get()};
    if (bidirectional) {
        celeg::launch_gqa_prefill_relative_bidirectional(args);
    } else {
        celeg::launch_gqa_prefill_relative(args);
    }
...
void run_int8(celeg::CudaStream& stream,
    ...
    const celeg::GqaContiguousInt8Args args{
        .query = dquery.data(),
        .kv = {.keys = dkeys.data(),
               .values = dvalues.data(),
        ...
        .stream = stream.get()};
    if (bidirectional) {
        celeg::launch_gqa_prefill_relative_bidirectional_int8(args);
    } else {
        celeg::launch_gqa_prefill_relative_int8(args);
    }
    check_output(output, stream, expected);
}
...
int main() {
    ...
    const std::array<float, 6> causal = cpu_reference(host_bias, false);
    run_bf16(stream, bias, causal, false);
    run_int8(stream, bias, causal, false);

    const std::array<float, 6> bidirectional = cpu_reference(host_bias, true);
    CELEG_TEST_CHECK(std::abs(bidirectional[0] - kValues[0]) > 0.5f);
    run_bf16(stream, bias, bidirectional, true);
    run_int8(stream, bias, bidirectional, true);
    return 0;
}
