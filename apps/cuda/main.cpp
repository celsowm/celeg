...
void print_memory_stats(const celeg::ModelMemoryStats& stats) {
    std::cerr << "memory.weights=" << celeg::text::format_bytes(stats.weights) << '\n'
              << "memory.kv_cache=" << celeg::text::format_bytes(stats.kv_cache) << '\n'
              << "memory.conv_state=" << celeg::text::format_bytes(stats.conv_state) << '\n'
              << "memory.activations=" << celeg::text::format_bytes(stats.activations) << '\n'
              << "memory.sampling=" << celeg::text::format_bytes(stats.sampling) << '\n'
              << "memory.matmul_workspace=" << celeg::text::format_bytes(stats.matmul_workspace) << '\n'
              << "memory.attention_workspace=" << celeg::text::format_bytes(stats.attention_workspace) << '\n'
              << "memory.total=" << celeg::text::format_bytes(stats.total()) << '\n';
}

celeg::CudaModelOptions build_model_options(const Args& args, bool is_gguf) {
...
std::string resolve_model_path(const celeg::app::PreparedRun& prepared) {
    if (prepared.is_gguf) return prepared.checkpoint_path.string();
...
/// Benchmark mode: report the prefill the caller already ran, then measure
/// decode throughput and expert-offload behaviour and exit. Extracted from
/// main() so the decode loop is a single branch in the orchestrator.
void run_benchmark(celeg::CudaModel& engine, const Args& args) {
...
int main(int argc, char** argv) {
    try {
        ...
        const celeg::CudaModelOptions model_options =
            build_model_options(args, prepared.is_gguf);
        const std::string model_path = resolve_model_path(prepared);
