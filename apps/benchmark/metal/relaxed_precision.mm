/// @file
/// Benchmark-only A/B for the Metal TensorOps relaxed-precision descriptor.
/// Baseline and candidate are identical 64x128xK64 Q4_K kernels; the candidate
/// changes only matmul2d_descriptor::relaxed_precision from false to true.
/// Every shape/token count must remain bit-exact before timing starts.
...
id<MTLBuffer> make_weights(id<MTLDevice> device, const Shape& shape,
                           uint32_t& row_bytes) {
    if (shape.cols % kQ4KBlockValues != 0) {
        throw std::runtime_error(std::string(shape.label) + " has partial Q4_K block");
    }
...
void require_bit_exact(id<MTLCommandQueue> queue,
    ...
    id<MTLBuffer> left = zero_buffer(queue.device, elements * sizeof(float));
    id<MTLBuffer> right = zero_buffer(queue.device, elements * sizeof(float));
    id<MTLCommandBuffer> command_buffer = [queue commandBuffer];
    id<MTLComputeCommandEncoder> encoder = [command_buffer computeCommandEncoder];
    encode_matmul(encoder, baseline, weights, input, left, tokens, shape, row_bytes);
    encode_matmul(encoder, relaxed, weights, input, right, tokens, shape, row_bytes);
    [encoder endEncoding];
    ...
    for (size_t index = 0; index < elements; ++index) {
        if (std::memcmp(a + index, b + index, sizeof(float)) != 0) {
            throw std::runtime_error(
                std::string("relaxed precision is not bit-exact for ") + shape.label +
...
int main() {
    try {
        ...
        if (!library) {
            throw std::runtime_error("Metal relaxed-precision shader failed: " +
                (error ? ns_string(error.localizedDescription) : "unknown"));
        }
        ...
        std::cout << "Metal TensorOps relaxed-precision A/B on "
                  << ns_string(device.name) << '\n';
        std::cout << "geometry=64x128xK64 threads=128 bit_exact=required\n\n";
        std::cout << std::left << std::setw(24) << "shape"
                  << std::right << std::setw(7) << "pp"
                  << std::setw(13) << "strict ms"
                  << std::setw(13) << "relaxed ms"
                  << std::setw(11) << "speedup" << '\n';
        ...
        for (const Shape& shape : kShapes) {
            ...
            for (uint32_t tokens : kTokenCounts) {
                ...
                const double strict_ms = time_kernel(queue, 5, iterations,
                    ^(id<MTLComputeCommandEncoder> encoder) {
                        encode_matmul(encoder, baseline, weights, input, output,
                                      tokens, shape, row_bytes);
                    });
                const double relaxed_ms = time_kernel(queue, 5, iterations,
                    ^(id<MTLComputeCommandEncoder> encoder) {
                        encode_matmul(encoder, relaxed, weights, input, output,
                                      tokens, shape, row_bytes);
                    });
                std::cout << std::left << std::setw(24) << shape.label
                          << std::right << std::setw(7) << tokens
                          << std::fixed << std::setprecision(3)
                          << std::setw(13) << strict_ms
                          << std::setw(13) << relaxed_ms
                          << std::setw(10) << (strict_ms / relaxed_ms) << "x\n";
            }
        }
        return 0;
