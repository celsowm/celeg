// Benchmark-only strict-precision Q4_K TensorOps kernel matching the current
// llama.cpp mul_mm decomposition: 64 output rows x 128 tokens x K32, 128
// threads, one 16-value dequantization chunk per thread. The descriptor keeps
// relaxed_precision=false; only the Q4_K chunk decoder/layout mirrors llama.cpp.

constant int kCelegLlamaK32TileRows = 64;
constant int kCelegLlamaK32TileTokens = 128;
constant int kCelegLlamaK32TileK = 32;
constant int kCelegLlamaK32Threads = 128;
constant int kCelegLlamaK32Chunk = 16;

inline uchar2 celeg_llama_scale_min_k4(int j, int k, device const uchar* q) {
    return j < 4
...
inline void celeg_q4k_store16_llama_strict(threadgroup half* destination,
    ...
    il &= 3;
    const uchar2 sc = celeg_llama_scale_min_k4(scale_index, il / 2, block + 4);
    ...
    #pragma unroll
    for (uint index = 0; index < kCelegLlamaK32Chunk; ++index) {
        destination[index] = static_cast<half>(
            dl * static_cast<float>(static_cast<uint>(q[index]) & mask) - ml);
    }
}

kernel void celeg_matmul_tensor_q4k_k32_llama_strict(
    ...
    const int row_offset = static_cast<int>(grid.x) * kCelegLlamaK32TileRows;
    const int token_offset = static_cast<int>(grid.y) * kCelegLlamaK32TileTokens;
    const int row_extent = min(kCelegLlamaK32TileRows,
                               static_cast<int>(output_rows) - row_offset);
    const int token_extent = min(kCelegLlamaK32TileTokens,
    ...
    auto weights_type = tensor<threadgroup half, dextents<int32_t, 2>, tensor_inline>(
        weights_tile,
        dextents<int32_t, 2>(kCelegLlamaK32TileK, kCelegLlamaK32TileRows),
        array<int32_t, 2>({1, kCelegLlamaK32TileK}));

    matmul2d<
        matmul2d_descriptor(kCelegLlamaK32TileTokens, kCelegLlamaK32TileRows,
    ...
    auto first_input = tensor(
        ...
        dextents<int32_t, 2>(min(kCelegLlamaK32TileK, static_cast<int>(cols)),
                             token_extent),
    ...
    constexpr int chunks_per_row = kCelegLlamaK32TileK / kCelegLlamaK32Chunk;
    ...
    threadgroup half* destination = weights_tile +
        tile_row * kCelegLlamaK32TileK + tile_chunk * kCelegLlamaK32Chunk;

    for (int offset = 0; offset < static_cast<int>(cols);
         offset += kCelegLlamaK32TileK) {
        const int source_column = offset + tile_chunk * kCelegLlamaK32Chunk;
        if (source_row < static_cast<int>(output_rows) &&
            source_column < static_cast<int>(cols)) {
            celeg_q4k_store16_llama_strict(
                destination,
                weights + static_cast<size_t>(source_row) * row_bytes,
                static_cast<uint>(source_column));
        } else {
            #pragma unroll
            for (int index = 0; index < kCelegLlamaK32Chunk; ++index) {
                destination[index] = static_cast<half>(0);
            }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        const int extent = min(kCelegLlamaK32TileK, static_cast<int>(cols) - offset);
