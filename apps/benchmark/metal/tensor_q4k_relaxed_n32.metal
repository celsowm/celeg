// Benchmark-only Q4_K TensorOps variant for small prefill batches.
// Concatenated after tensor.metal and tensor_q4k_relaxed.metal so it reuses
// the production decoder, row/K tile geometry, and relaxed numerical policy.

constant int kCelegSmallRelaxedTileTokens = 32;

template <typename Decoder>
void celeg_matmul_tensor_quantized_relaxed_n32(
    ...
    const int token_offset = static_cast<int>(grid.y) * kCelegSmallRelaxedTileTokens;
    const int row_extent = min(kCelegRelaxedTileRows,
                               static_cast<int>(output_rows) - row_offset);
    const int token_extent = min(kCelegSmallRelaxedTileTokens,
    ...
    matmul2d<
        matmul2d_descriptor(kCelegSmallRelaxedTileTokens, kCelegRelaxedTileRows,
...
kernel void celeg_matmul_tensor_q4k_relaxed_n32(
    ...
    celeg_matmul_tensor_quantized_relaxed_n32(
